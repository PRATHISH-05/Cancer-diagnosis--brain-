...
def generate_pdf_report(original_img, heatmap_img, highlighted_img, tumor_type, confidence, all_predictions, timestamp, filename):
    """Generate comprehensive PDF report with images and predictions"""
    if not REPORTLAB_AVAILABLE:
        return None, None
    
    try:
        pdf_filename = f"brain_tumor_report_{timestamp.strftime('%Y%m%d_%H%M%S')}.pdf"
        ...
        story.append(Paragraph("DETAILED PROBABILITY ANALYSIS", heading_style))
        
        pred_data = [['Tumor Type', 'Probability']]
        for idx in range(4):
        ...
        # Original Image
        story.append(Paragraph("Original MRI Image", styles['Heading3']))
        img_buffer = io.BytesIO()
        original_img.save(img_buffer, format='PNG')
        img_buffer.seek(0)
        img_path_orig = os.path.join(REPORTS_DIR, f"original_{timestamp.strftime('%Y%m%d_%H%M%S')}.png")
        ...
        story.append(Paragraph("Red areas indicate potential tumor regions; blue areas show normal tissue.", normal_style))
        img_path_heat = os.path.join(REPORTS_DIR, f"heatmap_{timestamp.strftime('%Y%m%d_%H%M%S')}.png")
        ...
        story.append(Paragraph("Yellow boxes and blue circles mark detected anomalies.", normal_style))
        img_path_high = os.path.join(REPORTS_DIR, f"highlighted_{timestamp.strftime('%Y%m%d_%H%M%S')}.png")
        ...
        if tumor_type == 'No Tumor':
            story.append(Paragraph(
                "✅ No Tumor Detected<br/>The MRI analysis indicates no visible tumor abnormalities. "
                "However, regular monitoring is recommended based on medical history.",
                normal_style
            ))
        else:
            story.append(Paragraph(
                f"⚠️ {tumor_type} Detected (Confidence: {confidence:.2f}%)<br/>"
                "IMPORTANT NEXT STEPS:<br/>"
                "1. Schedule consultation with neurologist or oncologist<br/>"
                "2. Obtain additional confirmatory imaging if recommended<br/>"
                "3. Discuss treatment options with medical specialists<br/>"
                "4. Review pathology reports if biopsy is performed<br/>"
                "5. Follow personalized treatment plan<br/><br/>"
        ...
        story.append(Paragraph(
            ...
            "(neurologists, radiologists, oncologists) for comprehensive medical evaluation and treatment decisions. "
...
def save_to_history(image_name, tumor_type, confidence, timestamp, all_predictions):
    """Save prediction to history with all prediction percentages"""
    history_data = []
    ...
    new_entry = {
        ...
        "Glioma%": round(all_predictions[0] * 100, 2),
        "Meningioma%": round(all_predictions[1] * 100, 2),
        "No Tumor%": round(all_predictions[2] * 100, 2),
        "Pituitary%": round(all_predictions[3] * 100, 2)
    }
...
if uploaded_file is not None:
    ...
    if model is None:
        ...
        with col1:
            ...
            st.text("Areas with high intensity (red) indicate potential anomalies", help="Laplacian edge detection")
            img_array = np.array(image.resize((224, 224)).convert('L')) / 255.0
            heatmap_img = create_heatmap_overlay(img_array, all_predictions)
            heatmap_display = heatmap_img.resize((220, 220))
        ...
        with col2:
            ...
            highlighted_display = highlighted_img.resize((220, 220))
        ...
        # Save to history
        save_to_history(uploaded_file.name, tumor_type, confidence, timestamp, all_predictions)
...
"""
        ...
        with col2:
            if REPORTLAB_AVAILABLE:
                if st.button("🔄 Generate PDF Report", key="gen_pdf"):
                    with st.spinner("⏳ Generating PDF..."):
                        pdf_filename, pdf_path = generate_pdf_report(
                            ...
                            tumor_type, confidence, all_predictions, timestamp, uploaded_file.name
                        )
                ...
                st.info("ℹ️ ReportLab not installed - PDF generation unavailable")
        ...
        with col3:
            ...
            st.download_button(
                ...
                file_name=f"tumor_data_{timestamp.strftime('%Y%m%d_%H%M%S')}.json",
