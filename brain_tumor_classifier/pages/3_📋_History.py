...
st.markdown('<h1 class="history-header">📋 Detection History</h1>', unsafe_allow_html=True)
...
if not history:
    ...
    for entry in history_sorted:
        df_display.append({
            ...
            'No Tumor %': entry.get('no_tumor', 'N/A'),
    ...
    for idx, entry in enumerate(history_sorted[:5]):
        ...
        <div class="history-card" style="border-left-color: {color};">
            ...
            <span class="history-text">🧬 <b>Predictions:</b> Glioma: {entry.get('glioma', 'N/A')}% | Meningioma: {entry.get('meningioma', 'N/A')}% | No Tumor: {entry.get('no_tumor', 'N/A')}% | Pituitary: {entry.get('pituitary', 'N/A')}%</span><br/>
    ...
    with col1:
        ...
        st.download_button(
            ...
            data=csv_data,
            file_name=f"detection_history_detailed_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            mime="text/csv"
        )
    ...
    with col3:
        ...
        for idx, entry in enumerate(history_sorted, 1):
            ...
            summary_text += f"  All Predictions: Glioma={entry.get('glioma', 'N/A')}%, Meningioma={entry.get('meningioma', 'N/A')}%, No Tumor={entry.get('no_tumor', 'N/A')}%, Pituitary={entry.get('pituitary', 'N/A')}%\n"
